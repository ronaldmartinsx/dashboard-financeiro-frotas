...
Recomendacao para o front-end: chame `opcoes_filtros()` **uma vez** no boot,
prefira `comparativo_anual` a montar a tabela metrica a
...
4. **Revisao humana**: nenhuma funcao da camada de dados aceita SQL vindo da UI.
   Os unicos identificadores interpolados (`faturamento_por_dimensao`,
   `custos_por_dimensao`) passam por **lista branca**; valor livre nunca vira SQL.
   `valores_distintos`, a terceira, saiu do modulo junto com as `listar_*` que a
   chamavam.
...
`receita.faturamento_por_dimensao` e `custos.custos_por_dimensao` levantam
`ValueError` para dimensao desconhecida.
